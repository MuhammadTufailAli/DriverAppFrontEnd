...
const MyBooking = ({navigation}) => {
  ...
  const getData = async () => {
    ...
    axios
      .get(
        `http://10.0.2.2:3001/driver/hireBy?driverUserName=${parsedDriver.userName}`,
      )
      .then(response => {
  ...
  const GetHistoryData = () => {
    axios
      .get(`/driver/driverAttendanceHistory?reqId=${data.data.reqId}`)
      .then(response => {
        if (response.data.code == 0) {
  ...
  const MarkAttendace = () => {
    ...
    axios
      .post('http://10.0.2.2:3001/driver/attendanceRequest', attendace)
      .then(response => {
  ...
  const AddExpense = () => {
    ...
    console.log(user);
    axios.put('http://10.0.2.2:3001/driver/addExpense', user).then(response => {
      if (response.data.code == 0) {
  ...
  const CancelBooking = () => {
    axios
      .get(
        `http://10.0.2.2:3001/carOwner/bookingcancel?reqId=${data.data.reqId}`,
      )
      .then(response => {
        if (response.data.code == 0) {
